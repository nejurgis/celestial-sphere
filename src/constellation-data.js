// ── constellation-data.js ────────────────────────────────────────────────
// Real star-position line data for the 12 zodiacal constellations, sourced
// from d3-celestial (github.com/ofrohn/d3-celestial, CC0/MIT, itself built
// from the Yale Bright Star Catalogue) — not approximated. Cross-checked
// against known catalog positions before use: Regulus (α Leo) at
// [152.093, 11.9672] matches its real RA/Dec (~10h08m, +11.97°) exactly,
// same for Hamal (α Ari) and Spica (α Vir).
//
// Each entry is one constellation's line figure as a list of "strands" —
// disconnected polylines, since most figures aren't a single unbroken
// path (e.g. Leo's mane vs. its tail-tuft). Each strand is an ordered list
// of [RA_degrees, Dec_degrees] points to connect in sequence. RA here can
// be negative (the source uses a -180..180 range) — normalize with
// ((deg % 360) + 360) % 360 when consuming.
